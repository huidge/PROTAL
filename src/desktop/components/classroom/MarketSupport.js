...
class MarketSupportComponent extends Component {
  constructor(props) {
    super(props);
    this.state = {
      options: [],
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      param.supportType = 'COST';
      param.phoneCode = prefix;
      param.costStartDate = values.costDate[0].format('YYYY-MM-DD HH:mm:ss');
      param.costEndDate = values.costDate[1].format('YYYY-MM-DD HH:mm:ss');
  ...
  render() {
    ...
    const selectPhoneTypeBefore = (
      getFieldDecorator('prefix', { initialValue: '86',
        ...
        <AntSelect defaultValue="86" style={{ width: 150 }}>
          {phoneOptions}
        </AntSelect>,
      )
    );
    ...
    return (
      <Content className={`${style.container} ${style['support-charge-rule']}`}>
        <Row className={`${style['background-white']}`}>
          <Row className={style['title-container']} ><span className={style['title-bar']}>会销支援</span></Row>
          <Row className={`${style.container}`} >
            <div style={{ marginLeft: '10%' }}>
              <Form onSubmit={this.handleSubmit}>
                ...
                <FormItem {...formItemLayout} label="会销主题" >
                  {getFieldDecorator('costTopic', {
                    rules: [{ required: true, message: '请输入会销主题', whitespace: true }],
                  })(
                    <Input placeholder=" " />,
                      )}
                </FormItem>
                <FormItem {...formItemLayout} label="参与人群主要类型" >
                  {getFieldDecorator('costParType', {
                    rules: [{ required: true, message: '请输入参与人群主要类型', whitespace: true }],
                  })(
                    <Input placeholder="如：企业老板、银行职员、理财顾问等" />,
                      )}
                </FormItem>
                <FormItem {...formItemLayout} className={style.formitem_sty} label="参与人数">
                  {getFieldDecorator('costParTotal', {
                    rules: [{ required: true, message: '请输入参与人数', whitespace: true, type: 'number' }],
                  })(
                ...
                <FormItem
                  ...
                  label="培训时间"
                >
                  {getFieldDecorator('costDate', rangeConfig)(
                    <RangePicker
                ...
                <FormItem {...formItemLayout} label="会销地点" >
                  {getFieldDecorator('part', {
                    rules: [
                      {required:true, message:'请选择会销地点'},
                    ],
                  })(
                ...
                <FormItem {...formItemLayout} label="会销目标" style={{ marginBottom: '2%' }}>
                  {getFieldDecorator('costTarget', {
                    rules: [{ required: true, message: '请输入会销目标', whitespace: true }],
                  })(
                ...
                <FormItem {...tailFormItemLayout}>
                  <Row gutter={24}>
                    ...
                    <Col span={14}>
                      <Button type="primary" style={{height: 40, width: 120, float: 'right' }} loading={loading} htmlType="submit" size="large" >提交申请</Button>
                    </Col>
                  </Row>
                </FormItem>
              </Form>
...
const MarketSupport = Form.create()(MarketSupportComponent);
export default connect()(MarketSupport);
