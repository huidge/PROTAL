...
import {Table, Form, Input, Row, Col, Button, Dropdown, Menu, Select, Modal, DatePicker} from 'antd';
...
class OrderBondsPersonal extends React.Component {
  ...
  componentWillMount() {
    ...
    service.fetchOrderReferralListService(params1).then((data)=>{
      if(data.success){
        this.setState({
          orderList:data
        });
  ...
  clickSearch(){
    ...
    params.orderNumber = this.props.form.getFieldValue('itemIntroduction');
    params.itemName =this.props.form.getFieldValue('applicantInsurantIntroduction');
    params.status =this.props.form.getFieldValue('statusIntroduction');
  ...
  pendingOrder(record){
    service.fetchOrderDetailPendingList({orderId:record.orderId}).then((data)=>{
      ...
      location.hash = '#/orderPending/OrderPendingTrail/bonds/introduction/'+pendingData[0].orderId+'/'+pendingData[0].pendingId;
    });
  }

  //查看详情/修改
  orderSelect(record){
    location.hash = `/order/orderBondsDetail/introduction/${record.orderId}`;
  }
          ...
          <div>
            <Form onSubmit={this.handleSearch} >
              <Row>
                <Col span={4} style={{paddingRight:'10px'}}>
                  <FormItem {...formItemLayout}  style={{fontSize:'16px'}}>
                    {getFieldDecorator('statusIntroduction')(
                      <Select
                        showSearch
                        style={{width:'100%'}}
                        placeholder='状态'>
                ...
                <Col span={4} style={{paddingRight:'10px'}}>
                  <FormItem {...formItemLayout} style={{marginBottom:'0'}}>
                    {getFieldDecorator('applicantInsurantIntroduction')(
              ...
              <Row>
                ...
                <Col span={4} style={{paddingRight:'10px'}}>
                  <FormItem {...formItemLayout} style={{marginBottom:'0'}}>
                    {getFieldDecorator('channelNameIntroduction')(
